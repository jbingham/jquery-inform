...
$el.val(data[key]);});}});}else{var data={};this.find(':input').each(function(){if($(this).attr('name'))
data[$(this).attr('name')]=getFormValue($(this));});return data;}},edited:function(isEdited){if(isEdited===undefined)
return isFormEdited(this);return this.each(function(){var $form=$(this);$form.data('inform').edited=isEdited;if(!isEdited)
methods.apply.apply($form,[$form]);formEdited($form,isEdited);});},enabled:function(isEnabled){var b=this.data('inform').enabled;if(isEnabled===undefined)
return(b===undefined?true:b);return this.each(function(){var $form=$(this);$form.data('inform').enabled=isEnabled;if(isEnabled){$form.find("input,textarea,select").not("input[type='checkbox'],input[type='radio']").filter(':not(.ui-inform-locked)').attr('readonly',false).removeClass('ui-inform-readonly');$form.find("input[type='checkbox'],input[type='radio'],select").filter(':not(.ui-inform-locked)').removeAttr('disabled').removeClass('ui-state-disabled');if(isFormEdited($form))
formEdited($form,true);}else{$form.find("input,textarea,select").not("input[type='checkbox'],input[type='radio']").filter(':not(.ui-inform-locked)').attr('readonly',true).addClass('ui-inform-readonly');$form.find("button,input[type='checkbox'],input[type='radio'],select").filter(':not(.ui-inform-locked)').attr('disabled',true).addClass('ui-state-disabled');}
...
return settings.successMessage.apply($form,[data]);return settings.successMessage;}
...
return settings.errorMessage.apply($form,[xhr,status,err]);return settings.errorMessage;}
function isFormEdited($form){var isEdited=$form.data('inform').edited;if(isEdited===undefined)
isEdited=false;$form.find('select,input,textarea').each(function(){if(isFieldEdited($(this))){isEdited=true;return false;}});return isEdited;}
