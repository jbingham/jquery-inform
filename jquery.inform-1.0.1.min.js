...
s.apply($form,[data]);},error:function(xhr,status,err){$form.css('cursor','default');methods.enabled.apply($form,[true]);if(e&&$.isFunction(e)){e.apply($form,[xhr,status,err]);}else if($.dialog){$('#smrtform_dialog').remove();$('body').append('<div id="smrtform_dialog" title="'
+settings.errorTitle+'"><p>'
+getErrorMessage($form,xhr,status,err)
+'</p></div>');$('#smrtform_dialog').dialog();}else{alert(getErrorMessage($form,xhr,status,err));}}});}
...
function fieldEdited($field,isEdited){var settings=$form.data('inform'),isFormEdited=isFormEdited($(this));formEdited(this,isFormEdited);if(settings.edit&&$.isFunction(settings.edit))
...
return true;methods.enabled.apply($form,[false]);formEdited($form,false);if(settings.ajax)
$.extend(opts,settings.ajax);if(settings.wrapEvents){s=opts.success;e=opts.error;$.extend(opts,{success:function(data){$form.css('cursor','default');methods.apply.apply($form,[]);methods.enabled.apply($form,[true]);if(s&&$.isFunction(s)){s.apply($form,[data]);}else if($.fn.dialog){$('#ui-inform-dialog').remove();$('body').append('<div id="ui-inform-dialog" title="'
+settings.successTitle+'"><p>'
+getSuccessMessage($form,data)+'</p></div>');$('#ui-inform-dialog').dialog();}else{alert(getSuccessMessage($form,data));}},error:function(xhr,status,err){$form.css('cursor','default');methods.enabled.apply($form,[true]);formEdited($form,true);if(e&&$.isFunction(e)){e.apply($form,[xhr,status,err]);}else if($.fn.dialog){$('#smrtform_dialog').remove();$('body').append('<div id="ui-inform-dialog" title="'
+settings.errorTitle+'"><p>'
+getErrorMessage($form,xhr,status,err)
