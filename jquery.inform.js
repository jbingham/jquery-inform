...
(function($) {
	...
	var methods = {
        ...
        edited: function(isEdited) {
        	if (isEdited === undefined)
        		return isFormEdited(this);        	
        	
			return this.each(function(){
				var $form = $(this);
				$form.data('inform').edited = isEdited;
				if (!isEdited)
					methods.apply.apply($form, [$form]);
				formEdited($form, isEdited);
			});
        },
		
        // Enable or disable the form. 
		enabled: function(isEnabled) {
			var b = this.data('inform').enabled;
			if (isEnabled === undefined) 
				return (b === undefined ? true : b);			
				
			return this.each(function(){
				var $form = $(this);
				$form.data('inform').enabled = isEnabled;
				...
				if (isEnabled) {
			        ...
			        $form.find("button,input[type='checkbox'],input[type='radio'],select")
		            	.filter(':not(.ui-inform-locked)')
		            	.attr('disabled', true)
    ...
    function isFormEdited($form) {
        var isEdited = $form.data('inform').edited;
        if (isEdited === undefined)
        	isEdited = false;
